...
class timedeltaTests(EqualityTests, TotalOrderingTests, VectorSpaceTests):
    ...
    def test_generic_2354_less_or_equal_consistent_with_multiplication(
        self, a: ScalarT, b: ClassUnderTest
    ) -> None:
        """0 <= a and 0 <= b ⇒ 0 <= a * b"""
        self.assertImplies(
            (self.scalar_one - self.scalar_one) <= a and self.zero <= b,
            self.zero <= a * b,
        )
...
@Given(
    {
        ClassUnderTest: st.integers(
            min_value=-864000000000, max_value=864000000000
        ).map(lambda i: datetime.timedelta(milliseconds=i)),
...
@Given(
    {
        ClassUnderTest: st.datetimes(
            min_value=datetime.datetime(1900, 1, 1),
            max_value=datetime.datetime(2100, 1, 1),
        ),
        VectorSpaceT: st.timedeltas(
            min_value=datetime.timedelta(-3650), max_value=datetime.timedelta(3650)
        ),
    }
)
...
@Given(
    {
        ClassUnderTest: st.dates(
            min_value=datetime.date(1900, 1, 1), max_value=datetime.date(2100, 1, 1)
        ),
        VectorSpaceT: st.integers(min_value=-3650, max_value=3650).map(
            lambda i: datetime.timedelta(i)
        ),
    }
)
